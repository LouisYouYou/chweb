      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {serviceTimes.map((s) => (
            <div
              key={s.id}
              className="group p-6 rounded-2xl border border-wine-100 hover:border-wine-300 hover:shadow-lg transition-all duration-300 hover:-translate-y-1"
            >
              ...
              <h3 className="font-semibold text-wine-900 mb-2 leading-tight">
                {locale === 'zh-TW' ? s.nameZh : s.nameEn}
              </h3>
              <p className="text-sm text-gray-500 mb-1">
                {locale === 'zh-TW' ? s.dayZh : s.dayEn}
              </p>
              <p className="text-lg font-bold text-wine-700 mb-3">{s.time}</p>
